<?php
namespace App\Controller;

use App\Entity\Movie;
use App\Repository\MovieRepository;
use Doctrine\ORM\EntityManagerInterface;
use Symfony\Component\HttpFoundation\Request;
use Symfony\Component\Routing\Annotation\Route;
use Sensio\Bundle\FrameworkExtraBundle\Configuration\Method;

class MovieController extends ApiController
{
    /**
    * @Route("/movies", methods="GET")
    */
    public function index(MovieRepository $movieRepository)
    {
        $movies = $movieRepository->transformAll();

        return $this->respond($movies);
    }


    /**
    * @Route("/movies", methods="POST")
    */
    public function create(Request $request, MovieRepository $movieRepository, EntityManagerInterface $em)
    {
        $request = $this->transformJsonBody($request);

        if (! $request) {
            return $this->respondValidationError('Please provide a valid request!');
        }

        // validate the title
        if (! $request->get('title')) {
            return $this->respondValidationError('Please provide a title!');
        }
         if (! $request->get('sinopsis')) {
            return $this->respondValidationError('Please provide a sinopsis!');
        }
         if (! $request->get('image')) {
            return $this->respondValidationError('Please provide a image!');
        }
       
        // persist the new movie
        $movie = new Movie;
        $movie->setTitle($request->get('title'));
        $movie->setSinopsis($request->get('sinopsis'));
        $movie->setImage($request->get('image'));
        $movie->setCount(0);
        $em->persist($movie);
        $em->flush();

        return $this->respondCreated($movieRepository->transform($movie));
    }

    /**
    * @Route("/movies/{id}/count", methods="POST")
    */
    public function increaseCount($id, EntityManagerInterface $em, MovieRepository $movieRepository)
    {
        /*if (! $this->isAuthorized()) {
            return $this->respondUnauthorized();
        }*/
        $movie = $movieRepository->find($id);

        if (! $movie) {
            return $this->respondNotFound();
        }

        $movie->setCount($movie->getCount() + 1);
        $em->persist($movie);
        $em->flush();

        return $this->respond([
            'count' => $movie->getCount()
        ]);
    }

     /**
    * @Route("/movies/{id}/update", methods="POST")
    */
    public function update(Request $request, $id, EntityManagerInterface $em, MovieRepository $movieRepository)
    {
       
        $request = $this->transformJsonBody($request);

        if (! $request) {
            return $this->respondValidationError('Realice una solicitud válida!');
        }

        // validate the title
        if (! $request->get('title')) {
            return $this->respondValidationError('Ingrese el título de la película');
        }
        if (! $request->get('sinopsis')) {
            return $this->respondValidationError('Ingrese la sinopsis de la pelicula');
        }
        if (! $request->get('image')) {
            return $this->respondValidationError('Ingrese la imagen de la pelicula');
        }

        $movie = $movieRepository->find($id);
        if (! $movie) {
            return $this->respondNotFound();
        }

        $movie->setTitle($request->get('title'));
          $movie->setSinopsis($request->get('sinopsis'));
            $movie->setImage($request->get('image'));
        //$movie->setCount($movie->getCount() + 1);
        $em->persist($movie);
        $em->flush();

        return $this->respond(['title' => $movie->getTitle()]);
        return $this->respond(['sinopsis' => $movie->getSinopsis()]);
        return $this->respond(['image' => $movie->getImage()]);
        
    /*     $serializer = new Serializer([new ObjectNormalizer()]);
        return new JsonResponse($serializer->normalize($movie, 'json', [ 'attributes' => [
            'id'
        ]]), 200);*/
      
    }

     /**
    * @Route("/movies/{id}", methods="DELETE")
    */
    public function delete($id, EntityManagerInterface $em, MovieRepository $movieRepository)
    {
        /*if (! $this->isAuthorized()) {
            return $this->respondUnauthorized();
        }*/

        $movie = $movieRepository->find($id);

        if (! $movie) {
            return $this->respondNotFound();
        }

        $em->remove($movie);
        $em->flush();

        return $this->respond([]);
    }
}